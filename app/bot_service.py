...
async def add_poll_in_db(message: Message, state: FSMContext, data: dict, answers: list[str]):
    ...
    await message.bot.send_poll(chat_id=message.from_user.id, question=data['title'], options=answers)
    await message.answer('<b>Опрос был отправлен на проверку, если пройдет - то будет отправлен на канал.</b>')
